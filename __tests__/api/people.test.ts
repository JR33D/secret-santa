  ...
  describe('GET /api/people', () => {
    ...
    it('filters people by pool_id when provided', async () => {
      ...
      expect(mockDb.all).toHaveBeenCalledWith(
        expect.stringContaining('WHERE people.pool_id = ?'),
        [1]
      );
    ...
    it('orders results by name', async () => {
      ...
      expect(mockDb.all).toHaveBeenCalledWith(
        expect.stringContaining('ORDER BY people.name'),
        []
      );
    });
  ...
  describe('POST /api/people', () => {
    it('creates a new person successfully', async () => {
      ...
      expect(mockDb.run).toHaveBeenCalledWith(
        'INSERT INTO people (name, email, pool_id) VALUES (?, ?, ?)',
        ['New Person', 'new@example.com', 1]
      );
