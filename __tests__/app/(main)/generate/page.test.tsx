...
describe('Generate Assignments Page', () => {
	...
	beforeEach(() => {
		...
		(api.apiPost as jest.Mock).mockResolvedValue({ success: true, message: 'Assignments generated successfully' });
	...
	it('loads pools on mount', async () => {
		render(<Page />);
		await waitFor(() => expect(api.apiGet).toHaveBeenCalledWith('/api/pools'));
	});
	...
	it('sends notifications when button is clicked', async () => {
		...
		expect(api.apiPost).toHaveBeenCalledWith(`/api/send-notifications/${new Date().getFullYear()}?pool_id=1`);
	});

	it('shows warning if pool has less than 2 members', async () => {
		(api.apiGet as jest.Mock).mockImplementation((url: string) => {
			if (url.startsWith('/api/pools')) return Promise.resolve([{ id: 1, name: 'Small Pool', member_count: 1 }]);
			return Promise.resolve([]);
		});
