...
describe('ReceiverWishlistTab Component', () => {
	...
	it('loads assignment and wishlist on mount', async () => {
		(api.apiGet as jest.Mock).mockResolvedValueOnce(mockAssignment).mockResolvedValueOnce(mockWishlistItems);
	...
	it('displays item images', async () => {
		...
		await waitFor(() => {
			...
			const bookImage = images.find((img) => img.getAttribute('src') === 'https://example.com/book.jpg');
			expect(bookImage).toBeInTheDocument();
		});
	});

	it('shows message when receiver has no wishlist items', async () => {
		(api.apiGet as jest.Mock).mockResolvedValueOnce(mockAssignment).mockResolvedValueOnce([]);
	...
	it('shows loading state', async () => {
		(api.apiGet as jest.Mock).mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve([]), 100)));
	...
	it('handles image load errors', async () => {
		...
		await waitFor(() => {
			const images = screen.getAllByRole('img');
			const bookImage = images.find((img) => img.getAttribute('src') === 'https://example.com/book.jpg') as HTMLImageElement;
