...
export async function POST(request: NextRequest, context: { params: Promise<{ year: string }> }) {
	try {
		...
		const existing = await db.get('SELECT COUNT(*) as count FROM assignments WHERE year = ? AND pool_id = ?', [parsedYear, parseInt(poolId)]);
       ...
       WHERE g.pool_id = ? AND rec.pool_id = ?`,
		...
		if (!assignments) {
			return Response.json(
				{
					success: false,
					message: 'Could not generate valid assignments with current restrictions. Adjust restrictions and try again.',
				},
				{ status: 400 },
			);
		}
...
function generateAssignments(people: { id: number }[], restrictions: Map<number, Set<number>>): Map<number, number> | null {
