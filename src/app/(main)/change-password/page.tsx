	...
	return (
		<div>
			<div className="text-center mb-8">
				<h1 className="text-3xl font-extrabold text-indigo-600 mb-2">{mustChange ? '⚠️ Change Password Required' : '🔒 Change Password'}</h1>
				<p className="text-gray-600">{mustChange ? 'You must change your temporary password before continuing' : 'Update your password'}</p>
			</div>

			<form onSubmit={handleSubmit} className="space-y-4">
				<div>
					<label htmlFor="current-password" className="block text-sm font-semibold text-gray-700 mb-2">Current Password</label>
					<input
						id="current-password"
						type="password"
			...
			{!mustChange && (
				<button onClick={() => router.push('/')} className="w-full mt-4 text-gray-600 hover:text-gray-800 text-sm">
					Cancel
				</button>
			)}
		</div>
