...
export default function ReceiverWishlistTab() {
    ...
    return (
      <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4">
        <p className="text-yellow-700">
          Your account is not linked to a person. Please contact an administrator.
        </p>
      </div>
    );
  }
        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-4">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-white border-2 border-indigo-200 p-5 rounded-lg shadow-sm hover:shadow-md transition"
              >
                ...
                {item.link && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-block mt-2 px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition text-sm"
                  >
                    🔗 View Link
                  </a>
                )}
